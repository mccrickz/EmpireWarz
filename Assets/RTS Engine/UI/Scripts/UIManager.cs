		...
		public void UpdateBuildingTasks (Building SelectedBuilding)
		{
			...
			if (SelectedBuilding.IsBuilt == true) {
				if (GameManager.PlayerFactionID == SelectedBuilding.FactionID) {
					...
					if (SelectedBuilding.gameObject.GetComponent<APC> ()) {
                        if (SelectedBuilding.APCMgr.CurrentUnits.Count > 0)
                        {
                            if (SelectedBuilding.APCMgr.EjectAllUnits == true)
                            {
                                AddTaskButtons(1);
                                ...
                                TaskButtons[0].gameObject.GetComponent<Image>().sprite = SelectedBuilding.APCMgr.EjectAllIcon;
                            ...
                            if (SelectedBuilding.APCMgr.EjectSingleUnit == true)
                            {
                                for (i = 0; i < SelectedBuilding.APCMgr.CurrentUnits.Count; i++)
                                {
                                    AddTaskButtons(1);
                        ...
                        if (SelectedBuilding.APCMgr.CurrentUnits.Count < SelectedBuilding.APCMgr.MaxAmount && SelectedBuilding.APCMgr.CanCallUnits == true)
                        {
                            AddTaskButtons(1);
            ...
            if (SelectionMgr.SelectedUnits.Count > 0)
            {
                ...
                if (SelectionMgr.SelectedUnits.Count == 1) //if only one unit is selected.
                {
                    ...
                    if (SelectionMgr.SelectedUnits[0].APCMgr)
                    {
                        ...
                        {
                            if(SelectionMgr.SelectedUnits[0].APCMgr.EjectAllUnits == true)
                            {
                                AddTaskButtons(1);

                                //first task is for release all units
                                AddTask(ref LastTaskID, 0, TaskManager.TaskTypes.APCRelease, SelectionMgr.SelectedUnits[0].APCMgr.EjectAllUnitsTaskCategory, SelectionMgr.SelectedUnits[0].APCMgr.EjectAllIcon);
                            }

                            if (SelectionMgr.SelectedUnits[0].APCMgr.EjectSingleUnit == true)
                            {
                        ...
                        if (SelectionMgr.SelectedUnits[0].APCMgr.CurrentUnits.Count < SelectionMgr.SelectedUnits[0].APCMgr.MaxAmount && SelectionMgr.SelectedUnits[0].APCMgr.CanCallUnits == true)
                        {
                            AddTaskButtons(1);
